...
class feature_generator(BaseEstimator, TransformerMixin):
    ...
    def fit(self, open_price,high_price,low_price,close_price,vol, y=None):
        return self
    def transform(self,df,window_len=20):
        """返回numpy数组"""
...
class feature_engineering():
    ...
    def __init__(self,func_list):
        self.function_list=func_list
    
    def generate_transform_list(self,func_list,feature_generator):
        my_transform_list=[]
        name_list=[]
        for func in func_list:
            my_transform_list.append([func.__name__,feature_generator(func)])
            name_list.append(func.__name__)
        return my_transform_list,name_list
    
    def output_feature(self):
        my_features=my_features_functions()
        #获取所有特征工程函数
        my_func_name_list=my_features_functions.get_all_methold()
